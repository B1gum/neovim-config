...
M = {
    ...
    {f(function(_, snip)
        ...
        local rows = tonumber(snip.captures[2])
        local format_type = snip.captures[3]

        -- Validate captures
        if not cols then
            print("Error: First capture (number of columns) is not a valid number.")
            return "c" -- Default to single column
        end

        -- Determine column format
        if format_type == "a" then
            if cols < 1 then
                print("Error: Number of columns must be at least 1 for augmented arrays.")
                return "c"
            end
            return string.rep("c", rows - 1) .. "|c"
        else
            return string.rep("c", cols)
        end
      end),
      d(1, generate_array_content)  -- Use generate_array_content instead of generate_matrix
    }),
